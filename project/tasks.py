...
job_hdr."DATE_START" as date_start, 1 as company_id, (select id from project_project where analytic_account_id =aa.parent_id) as project_id, job_hdr."DESCRIPTION" as description,
...
from (select distinct "AA", position(E'\\' in "AA") as "SUBACCOUNT", "PHASE_DESCR", "PROJECT", "PHASE", "PRJ_DESCR", "DATE_CREATED", "JOB", "SUFFIX",
    ...
    from "gss_ANALYTIC_ACCOUNTS" where trim("JOB")<>'' and position(E'\\' in "AA")>0 order by "PROJECT","PHASE","JOB") as aa
left join "gss_V_JOB_HEADER" as job_hdr on job_hdr."JOB"=aa."JOB" and job_hdr."SUFFIX"=aa."SUFFIX" 
